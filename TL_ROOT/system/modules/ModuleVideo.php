 ...
 * @package    Controller
 */
class ModuleVideo extends Frontend
{
	...
	public function getSearchablePages($arrPages, $intRoot=0)
	{
		...
		// Get all categories
		$objFaq = $this->Database->execute("SELECT id, jumpTo FROM tl_video_category");
		...
		while ($objFaq->next())
		{
			...
			// Get items
			$objItem = $this->Database->prepare("SELECT * FROM tl_video WHERE pid=? AND published=1 ORDER BY sorting")
									  ->execute($objFaq->id);
